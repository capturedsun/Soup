...
std::vector<GumboNode*> CSelector::matchAll(GumboNode* root)
{
	std::vector<GumboNode*> ret;
	matchAllInto(root, ret);
	return ret;
}

void CSelector::matchAllInto(GumboNode* root, std::vector<GumboNode*>& nodes)
{
	if (match(root))
	{
	...
	{
		GumboNode* child = (GumboNode*) root->v.element.children.data[i];
